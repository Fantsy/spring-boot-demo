...
public class ApiJobSimple {
    ...
    private static LiteJobConfiguration configuration() {
        ...
        int shardingTotalCount = 3; //作业分片总数,如果一个作业启动超过作业分片总数的节点，
        // 只有 shardingTotalCount 会执行作业.换句话说：当服务器数量大于分片总数，那么不是所有服务器都将会执行，而是根据分片总数来定。
